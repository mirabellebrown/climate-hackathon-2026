...
import { appendTurn, clearConversation } from "./conversation";
import { calculateImpact, compareUsage } from "./impact";
import { recordRoute, resetSession } from "./session";
import type { RouteResult, Tier, TokenUsage } from "./types";
...
/** Alternating light → medium → heavy (Flash Lite / Flash / Pro), seven of each. */
const DEMO_SPECS: DemoSpec[] = [
  {
    ...
    reason: "A short factual explanation fits Flash Lite.",
  ...
  },
  {
    ...
    reason: "Comparative product advice needs a bit more reasoning than Flash Lite.",
  ...
  },
  {
    ...
    reason: "System design with multi-axis tradeoffs warrants Gemini Pro.",
  ...
  },
  {
    ...
    reason: "Coding with explanation fits Flash.",
  ...
  },
  {
    ...
    reason: "A brief checklist is Flash Lite territory.",
  ...
  },
  {
    ...
    reason: "Short professional writing with tone control fits Flash.",
  ...
  },
  {
    ...
    reason: "ESG boundary judgment calls for Pro-level nuance.",
  ...
  },
  {
    ...
    reason: "Trivial unit conversion routes to Flash Lite.",
  ...
  },
  {
    ...
    reason: "Domain explainer with structure fits Flash.",
  ...
  },
  {
    ...
    answer: "Classify each request by complexity and risk.\n\n| Tier | When | SLO notes |\n| --- | --- | --- |\n| Flash Lite | FAQ, formatting | p95 < 1.5s |\n| Flash | Coding, summaries | quality review sample 5% |\n| Pro | Architecture, legal-ish | human escalate path |\n\nAdd classifiers with fallbacks, shadow-score Pro on a sample, and cap Pro share by budget.",
  ...
  },
  {
    ...
    reason: "Simple recall question for Flash Lite.",
  ...
  },
  {
    ...
    reason: "Short nuanced FAQ still fits Flash Lite.",
  ...
  },
  {
    ...
    answer: "Disclose **methodology version**, included scopes (inference vs training), and uncertainty.\n\nUse intensity metrics (g CO₂e per 1k requests) alongside totals; separate market-based electricity claims. Avoid implying equal answer quality between routed and always-Pro baselines. Point readers to exclusions (devices, network) so the narrative stays decision-useful rather than absolute.",
    reason: "Regulated disclosure language calls for Pro.",
  ...
  },
  {
    ...
    reason: "Trivial expansion for Flash Lite.",
  ...
  },
  {
    ...
    reason: "Product/ops comparison fits Flash.",
    generation: { inputTokens: 340, outputTokens: 300 },
    classifier: { inputTokens: 205, outputTokens: 30 },
  },
  {
    tier: "heavy",
    prompt: "Draft an RFC for retiring always-Pro defaults in favor of tiered routing across 12 internal tools.",
    answer: "## Motivation\nAlways-Pro defaults inflate spend and environmental load without measured quality gains.\n\n## Proposal\n1. Shared classifier service with per-tool allowlists.\n2. Gradual rollout: shadow mode → canary → default.\n3. Quality gates: human eval + automatic regression on golden prompts.\n4. Kill switch back to Pro per tool.\n\n## Risks\nMisroutes on regulated content; mitigate with forced-Pro tags and audit logs.",
    reason: "Org-wide RFC with rollout and risk needs Pro depth.",
    generation: { inputTokens: 1520, outputTokens: 1720 },
    classifier: { inputTokens: 280, outputTokens: 44 },
  },
];

function buildRouteResult(spec: DemoSpec): RouteResult {
  return {
    answer: spec.answer,
    routing: {
      tier: spec.tier,
      reason: spec.reason,
      model: MODELS[spec.tier].id,
      modelName: MODELS[spec.tier].name,
      classifierModel: CLASSIFIER_MODEL,
      classifierFallback: false,
      baselineModel: BASELINE_MODEL.id,
    },
    usage: compareUsage(spec.generation, spec.classifier),
    impact: calculateImpact(spec.tier, spec.generation, spec.classifier),
    truncated: false,
  };
}

/** Synthesize RouteResult-shaped demo turns (no API calls). */
export function buildDemoTurns(): { id: string; prompt: string; result: RouteResult }[] {
  return DEMO_SPECS.map((spec, index) => ({
    id: `demo-turn-${String(index + 1).padStart(2, "0")}`,
    prompt: spec.prompt,
    result: buildRouteResult(spec),
  }));
}

/**
 * Replace browser conversation + session totals with ~21 alternating-tier demo turns.
 * Uses the same localStorage keys as live chat (`recordRoute` / `appendTurn`).
 */
export function loadDemoData(): { turns: number; light: number; medium: number; heavy: number } {
  const turns = buildDemoTurns();
  resetSession();
  clearConversation();
  for (const turn of turns) {
    appendTurn({ id: turn.id, prompt: turn.prompt, result: turn.result });
    recordRoute(turn.prompt, turn.result);
  }
  const counts = { turns: turns.length, light: 0, medium: 0, heavy: 0 };
  for (const turn of turns) counts[turn.result.routing.tier] += 1;
  return counts;
}
