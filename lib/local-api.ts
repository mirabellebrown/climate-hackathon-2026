...
export function requireLocalRequest(request: Request) {
  ...
  if (!["localhost", "127.0.0.1", "[::1]"].includes(url.hostname)) throw new RouteFailure("LOCAL_ONLY", "GreenRoute runs on your own computer. Use its localhost address.", 403, "request");
