...
export function SessionPanel({ disabled }: { disabled: boolean }) {
  ...
  return <section className="session-panel" aria-labelledby="session-title">
    <div className="session-intro"><span className="session-icon"><Sprout size={22} /></span><div><h2 id="session-title">Little by little, it adds up.</h2><p>Your browser session · {session.requests} completed {session.requests === 1 ? "request" : "requests"}</p></div></div>
