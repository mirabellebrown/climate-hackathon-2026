...
export function Methodology() {
  return <details className="methodology" id="methodology">
    ...
    <div className="methodology-content">
      <div><h3>Tokens in. Estimates out.</h3><p>Energy = input tokens × 0.000135 Wh + output tokens × 0.00288 Wh, scaled by model. Sonnet = 1×, Haiku = 0.5×, Opus = 2×, and Flash Lite = 0.25×. Routing includes classifier tokens plus every token Claude Code reports for the run, per model, with cache reads and cache writes counted as full input.</p><p>The Sonnet factors derive from modeled Claude 3.7 results in Jegham et al. (2025), via claude-carbon. Applying them to today’s models and scaling the other families are assumptions, not direct measurements.</p></div>
      ...
      <div><h3>Useful perspective. Real uncertainty.</h3><p>These are order-of-magnitude inference estimates. Actual hardware, batching, grid mix, location, caching, and answer length vary. Training, hardware manufacturing, your device, and network use are excluded. The counterfactual does not establish equal answer quality.</p><p>Gemini and your Claude Code receive your prompt. The dashboard only receives token counts; answers stay in your terminal. No second answer is generated for comparison.</p></div>
    </div>
