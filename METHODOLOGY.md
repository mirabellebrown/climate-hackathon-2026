...
**Model families.** Energy scales are assigned per family: Haiku/Flash Lite 0.5, Sonnet/Flash 1, Opus/Pro 2. The Gemini rows reuse the Claude tier scales, matching `feat/claude-api-router` so both branches report comparable figures. This is an assumption, not a measurement of any Gemini model. The always-biggest baseline is the heavy model of whichever vendor answered (Opus for Claude, Pro for Gemini). A classifier is priced at its own family's scale: Flash Lite keeps the 0.25 assumption, and a Claude Haiku classifier is priced at the light scale.
